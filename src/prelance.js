...
export function analisarPreLance(campos) {
  const nomes = ['pesoTotal', 'pesoMetalMinimo', 'teor', 'kitco', 'desconto', 'precoVendaOuro', 'lance', 'custos', 'tarifaPercentual'];
  ...
  if (v.kitco <= 0 || v.desconto < 0 || v.desconto >= v.kitco || v.precoVendaOuro <= 0) throw new Error('Confira os preços do ouro e o desconto.');
  ...
  const refino = ouroFino * 0.03 * (v.kitco - v.desconto);
  const receita = ouroFino * v.precoVendaOuro;
  const tarifa = v.lance * v.tarifaPercentual / 100;
  const custo = v.lance + tarifa + v.custos + refino;
  const lucro = receita - custo;
  const lanceMaximo = (receita / 1.15 - v.custos - refino) / (1 + v.tarifaPercentual / 100);
  if (![ouroFino, refino, receita, tarifa, custo, lucro, lanceMaximo].every(Number.isFinite)) throw new Error('Valores grandes demais para cálculo seguro.');
  return { ouroFino, refino, receita, tarifa, custo, lucro, lanceMaximo: Math.max(0, lanceMaximo),
